...
<form action="/Horario/{{ $horario->id }}" method="POST"><br>
    @csrf
    @method('PUT')
            ...
            <div class="form-group{{ $errors->has('id_ruta') ? ' has-danger' : '' }}">
                <select class="form-control{{ $errors->has('id_ruta') ? ' is-invalid' : '' }}" name="id_ruta" id="input-id_ruta" required>
                    ...
                    @foreach($rutas as $ruta)
                        <option value="{{ $ruta->id }}" {{ old('id_ruta', $horario->id_ruta) == $ruta->id ? 'selected' : '' }}>
            ...
            <div class="form-group{{ $errors->has('id_minibus') ? ' has-danger' : '' }}">
                <select class="form-control{{ $errors->has('id_minibus') ? ' is-invalid' : '' }}" name="id_minibus" id="input-id_minibus" required>
                    ...
                    @foreach($asignaciones as $asignacion)
                        <option value="{{ $asignacion->id }}" {{ old('id_minibus', $horario->id_minibus) == $asignacion->id ? 'selected' : '' }}>
    ...
    <div class="row">
        ...
        <div class="col-sm-4">
            <div class="form-group{{ $errors->has('Fecha') ? ' has-danger' : '' }}">
                <input class="form-control{{ $errors->has('Fecha') ? ' is-invalid' : '' }}" name="Fecha" id="input-Fecha" type="date" value="{{ old('Fecha', $horario->Fecha) }}" required />
                @if ($errors->has('Fecha'))
    ...
    <div class="row">
        ...
        <div class="col-sm-4">
            <div class="form-group{{ $errors->has('Hora') ? ' has-danger' : '' }}">
                <input class="form-control{{ $errors->has('Hora') ? ' is-invalid' : '' }}" name="Hora" id="input-Hora" type="time" value="{{ old('Hora', $horario->Hora) }}" required />
                @if ($errors->has('Hora'))
    ...
    <div class="container">
        <a href="/Horario" class="btn btn-warning" style="margin-left: 150px" tabindex="5">CANCELAR</a>
