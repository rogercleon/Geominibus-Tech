...
@section('content')
    <h2>Actualizacion de Agencias</h2>
    <form action="/Agencia/{{$agencia->id}}" method="POST"><br>
        @csrf
        @method('put')
        <div class="row">
            ...
            <div class="col-sm-7">
                <div class="form-group{{ $errors->has('Agencia') ? ' has-danger' : '' }}">
                    <input class="form-control{{ $errors->has('Agencia') ? ' is-invalid' : '' }}" name="Agencia" id="input-Agencia" type="text" placeholder="{{ __('Agencia') }}" value="{{$agencia->Agencia}}" required="true" aria-required="true" />
